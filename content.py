...
LEVELS = [
    ...
    dict(title="Фильтрация", topic="WHERE · AND · OR · IN · LIKE", theory="WHERE оставляет строки, которые удовлетворяют условию. Условия можно соединять через AND и OR; LIKE ищет по шаблону, IN проверяет набор значений.", example="SELECT name\nFROM users\nWHERE age >= 18;", tasks=[
        ...
        ("Жители Берлина · финал", "Покажи имена и возраст пользователей из Берлина. В столбце city город записан как Berlin.", "SELECT name, age FROM users WHERE city = 'Berlin'", "Сравни столбец city со значением 'Berlin'.", "Строковое значение заключи в одинарные кавычки.", "SELECT name, age FROM users WHERE city = 'Berlin';", "Условие по городу оставило только подходящих пользователей."),
    ]),
...
INSERT INTO users VALUES (1,'Amalia','amalia@example.com',25,'Berlin'),(2,'Leo','leo@example.com',17,'Vienna'),(3,'Alice','alice@example.com',31,'Berlin'),(4,'Emma','emma@example.com',28,'Prague'),(5,'Noah','noah@example.com',22,'Vienna');
...
INSERT INTO departments VALUES (1,'Разработка','Berlin'),(2,'Продажи','Vienna'),(3,'Поддержка','Bratislava');
CREATE TABLE employees(id INTEGER PRIMARY KEY, name TEXT, department_id INTEGER, manager_id INTEGER, salary INTEGER);
INSERT INTO employees VALUES (1,'Sofia',1,NULL,120000),(2,'Lucas',1,1,90000),(3,'Emilia',2,NULL,95000),(4,'Daniel',2,3,65000),(5,'Olivia',3,NULL,70000);
