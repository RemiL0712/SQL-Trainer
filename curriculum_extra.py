...
EXTRA = [
    ...
    [
        ...
        ("Страница каталога", "Отсортируй товары по id по возрастанию. Покажи только name второго и третьего товара в этом порядке.", "SELECT name FROM products ORDER BY id LIMIT 2 OFFSET 1", "После FROM products добавь ORDER BY id.", "LIMIT 2 означает взять две строки, OFFSET 1 — пропустить одну. После OFFSET пишется число, а не имя столбца.", "ORDER BY id задаёт порядок, OFFSET 1 пропускает первый товар, LIMIT 2 оставляет следующие два."),
...
ADVANCED = [
    ...
    dict(title="Изменение данных", topic="INSERT · UPDATE · DELETE", theory="INSERT добавляет строки, UPDATE меняет существующие, DELETE удаляет. В этом модуле каждая попытка выполняется в отдельной копии учебной базы. Ни аккаунт, ни другие упражнения не меняются.", example="UPDATE inventory\nSET quantity = 5\nWHERE id = 3;", tasks=[
        ...
        ("Пополнение малых остатков", "Увеличь quantity на 5 у всех позиций, где quantity меньше 5.", "UPDATE inventory SET quantity = quantity + 5 WHERE quantity < 5", "В SET можно использовать прежнее значение.", "Условие quantity < 5 помести в WHERE.", "UPDATE поддерживает вычисления на основе текущих значений."),
    ...
    dict(title="Аналитические задачи", topic="Несколько таблиц · CTE · окна", theory="В реальной работе запросы связывают таблицы, агрегируют данные и сравнивают результат между группами. Сначала определяй зерно результата: одна строка на заказ, пользователя или категорию.", example="SELECT u.name, SUM(o.total) AS revenue\nFROM users u JOIN orders o ON u.id=o.user_id\nGROUP BY u.id;", tasks=[
        ...
        ("Число дорогих товаров", "Покажи category и expensive_count — число товаров дороже 1000 в каждой категории.", "SELECT category, SUM(CASE WHEN price > 1000 THEN 1 ELSE 0 END) AS expensive_count FROM products GROUP BY category", "Группируй по category.", "Для условного подсчёта используй SUM(CASE ...).", "Условная агрегация считает только дорогие товары внутри каждой категории."),
